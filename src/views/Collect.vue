  ...
  methods: {
    ...
    getApiType(){
            ...
            refreshCollectionTypesByKey(this.currentCollectionApi.key).then(res => {
              this.apiTypes = res
              for (let apiType of this.apiTypes) {
                for (let bindId of this.currentCollectionApi.bindId) {
                  if (bindId.apiId === apiType.id) {
                    bindId.apiName = apiType.name;
                    break; // 找到对应的 bindId 后结束循环
                  }
                }
              }
            })
